...
namespace Program.DAL.Repositories
{
    public class NutritionRepository
    {
        ...
        public List<Nutrition> GetNutrionsByCategoryName(string categoryName,int pageNumber)
        {
            return db.Nutritions.Include(x => x.Category).AsNoTracking().Where(x => x.Category.CategoryName == categoryName).Skip((pageNumber - 1) * 9).Take(9).ToList();
        }
        ...
        {
            return db.Nutritions.Include(x => x.Category).AsNoTracking().Where(x => x.UserID == null || x.UserID == userID).Skip((pageNumber - 1) * 9).Take(9).ToList();
        }
