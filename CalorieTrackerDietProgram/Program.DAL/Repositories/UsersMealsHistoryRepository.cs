...
namespace Program.DAL.Repositories
{
    public class UsersMealsHistoryRepository
    {
        ...
        public bool IsMealAlreadyAdded(int mealID,DateTime date)
        {
            return db.UsersMealsHistories.Any(x => x.MealID == mealID && x.CreatedDate.Date == date.Date&&x.IsActive==true);
        }
        ...
        public List<UsersMealsHistory> GetDaysUserMealHistoryOfMeal(int mealid,DateTime date,AppUser user)
        {
            return db.UsersMealsHistories
                .Where(x => x.MealID == mealid && x.CreatedDate.Date==date.Date && x.IsActive==true&& x.UserID==user.ID)
                .ToList();
        }
